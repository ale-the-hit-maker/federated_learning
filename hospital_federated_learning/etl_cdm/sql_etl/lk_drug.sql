...
-- Rule 1
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_prescriptions_clean;
CREATE TABLE @etl_database.@etl_schema.lk_prescriptions_clean AS
SELECT
    ...
    src.ndc::TEXT               AS ndc_source_code, -- ndc was used for automatic/manual mapping,
    'NDC'                       AS ndc_source_vocabulary,
    src.form_val_disp           AS form_val_disp,
    SUBSTRING(src.form_val_disp FROM '[-]?[\d]+[.]?[\d]*')::NUMERIC AS quantity,
    -- COALESCE(
...
FROM
    @etl_database.@etl_schema.src_prescriptions src -- pr
LEFT JOIN
    @etl_database.@etl_schema.src_pharmacy pharm
...
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_pr_ndc_concept;
CREATE TABLE @etl_database.@etl_schema.lk_pr_ndc_concept AS
SELECT DISTINCT
...
FROM
    @etl_database.@etl_schema.lk_prescriptions_clean src -- pr
LEFT JOIN
...
-- Rule 1
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_pr_gcpt_concept;
CREATE TABLE @etl_database.@etl_schema.lk_pr_gcpt_concept AS
SELECT DISTINCT
...
-- Rule 1
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_pr_route_concept;
CREATE TABLE @etl_database.@etl_schema.lk_pr_route_concept AS
SELECT DISTINCT
...
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_drug_mapped;
CREATE TABLE @etl_database.@etl_schema.lk_drug_mapped AS
SELECT
    src.hadm_id                                     AS hadm_id,
    src.subject_id                                  AS subject_id,
    ...
    src.trace_id                    AS trace_id
FROM
    @etl_database.@etl_schema.lk_prescriptions_clean src
LEFT JOIN
    @etl_database.@etl_schema.lk_pr_ndc_concept vc_ndc
        ON  src.ndc_source_code = vc_ndc.source_code
        AND vc_ndc.target_concept_id IS NOT NULL
LEFT JOIN
    @etl_database.@etl_schema.lk_pr_gcpt_concept vc_gcpt
        ON  src.gcpt_source_code = vc_gcpt.source_code
        AND vc_gcpt.target_concept_id IS NOT NULL
LEFT JOIN
    @etl_database.@etl_schema.lk_pr_route_concept vc_route
