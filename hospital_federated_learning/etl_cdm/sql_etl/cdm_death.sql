...
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_death_adm_mapped CASCADE;

CREATE TABLE @etl_database.@etl_schema.lk_death_adm_mapped AS
SELECT DISTINCT
    src.subject_id,
...
INSERT INTO @etl_database.@etl_schema.cdm_death
SELECT
    per.person_id       AS person_id,
    CAST(CASE
        WHEN src.deathtime <= src.dischtime THEN src.deathtime
        ELSE src.dischtime
    END AS DATE)                            AS death_date,
    CASE
        WHEN src.deathtime <= src.dischtime THEN src.deathtime
        ELSE src.dischtime
...
FROM
    @etl_database.@etl_schema.lk_death_adm_mapped src
INNER JOIN
