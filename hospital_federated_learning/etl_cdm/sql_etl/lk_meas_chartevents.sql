...
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_chartevents_clean;
CREATE TABLE @etl_database.@etl_schema.lk_chartevents_clean AS
SELECT
    ...
    CASE
        WHEN TRIM(src.value) ~ '^[-]?[0-9]+[.]?[0-9]*[ ]*[a-z]+$' THEN -- 7.5 mm etc
            (regexp_match(src.value, '[-]?[0-9]+[.]?[0-9]*'))[1]::FLOAT
        ELSE
            src.valuenum
    END                             AS valuenum,
    CASE
        WHEN TRIM(src.value) ~ '^[-]?[0-9]+[.]?[0-9]*[ ]*[a-z]+$' THEN -- 7.5 mm etc
            (regexp_match(src.value, '[a-z]+'))[1]
        ELSE
            src.valueuom
...
FROM
    @etl_database.@etl_schema.src_chartevents src -- ce
INNER JOIN
...
WHERE
    ...
    OR (di.label LIKE '%Temperature'
        AND CASE
            WHEN valueuom LIKE '%F%' THEN (valuenum - 32) * 5.0 / 9.0
            ELSE valuenum
...
DROP TABLE IF EXISTS @etl_database.@etl_schema.tmp_chartevents_code_dist;
CREATE TABLE @etl_database.@etl_schema.tmp_chartevents_code_dist AS
...
SELECT
    ...
    COUNT(*)                    AS row_count
FROM
    @etl_database.@etl_schema.lk_chartevents_clean
GROUP BY
    itemid,
    source_code,
    source_label
...
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_chartevents_concept;
CREATE TABLE @etl_database.@etl_schema.lk_chartevents_concept AS
SELECT
    src.itemid                  AS itemid,
...
FROM
    @etl_database.@etl_schema.tmp_chartevents_code_dist src
LEFT JOIN
...
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_chartevents_mapped;
CREATE TABLE @etl_database.@etl_schema.lk_chartevents_mapped AS
SELECT
    ...
    CASE
        WHEN src.valuenum IS NULL AND src.value IS NOT NULL THEN
    ...
    src.trace_id            AS trace_id
FROM
    @etl_database.@etl_schema.lk_chartevents_clean src -- ce
LEFT JOIN
    @etl_database.@etl_schema.lk_chartevents_concept c_main -- main
        ON c_main.source_code = src.source_code
        AND c_main.source_vocabulary_id = 'mimiciv_meas_chart'
LEFT JOIN
    @etl_database.@etl_schema.lk_chartevents_concept c_value -- values for main
...
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_chartevents_condition_mapped;
CREATE TABLE @etl_database.@etl_schema.lk_chartevents_condition_mapped AS
SELECT
    src.subject_id                              AS subject_id,
    src.hadm_id                                 AS hadm_id,
    src.stay_id                                 AS stay_id,
...
FROM
    @etl_database.@etl_schema.lk_chartevents_clean src -- ce
INNER JOIN
    @etl_database.@etl_schema.lk_chartevents_concept c_main -- condition domain from values, mapped
