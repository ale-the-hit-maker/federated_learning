...
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_observation_clean;
CREATE TABLE @etl_database.@etl_schema.lk_observation_clean AS
-- rule 1, insurance
SELECT
...
INSERT INTO @etl_database.@etl_schema.lk_observation_clean
SELECT
    src.subject_id                  AS subject_id,
    src.hadm_id                     AS hadm_id,
    ...
    src.trace_id                    AS trace_id
FROM
    @etl_database.@etl_schema.src_drgcodes src -- drg
INNER JOIN
...
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_obs_admissions_concept;
CREATE TABLE @etl_database.@etl_schema.lk_obs_admissions_concept AS
SELECT DISTINCT
...
FROM
    @etl_database.@etl_schema.lk_observation_clean src
LEFT JOIN
...
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_observation_mapped;
CREATE TABLE @etl_database.@etl_schema.lk_observation_mapped AS
SELECT
...
FROM
    @etl_database.@etl_schema.lk_observation_clean src
LEFT JOIN
    @etl_database.@etl_schema.lk_obs_admissions_concept lc
