...
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_micro_cross_ref CASCADE;
CREATE TABLE @etl_database.@etl_schema.lk_micro_cross_ref AS
SELECT
...
FROM
    @etl_database.@etl_schema.src_microbiologyevents src -- mbe
;
...
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_micro_hadm_id CASCADE;
CREATE TABLE @etl_database.@etl_schema.lk_micro_hadm_id AS
SELECT
...
FROM
    @etl_database.@etl_schema.lk_micro_cross_ref src
INNER JOIN
    @etl_database.@etl_schema.lk_admissions_clean adm
...
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_meas_organism_clean CASCADE;
CREATE TABLE @etl_database.@etl_schema.lk_meas_organism_clean AS
SELECT DISTINCT
...
FROM
    @etl_database.@etl_schema.src_microbiologyevents src -- mbe
INNER JOIN
    @etl_database.@etl_schema.lk_micro_cross_ref cr
...
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_specimen_clean CASCADE;
CREATE TABLE @etl_database.@etl_schema.lk_specimen_clean AS
SELECT DISTINCT
    src.subject_id                              AS subject_id,
    src.hadm_id                                 AS hadm_id,
...
FROM
    @etl_database.@etl_schema.lk_meas_organism_clean src -- mbe
INNER JOIN
    @etl_database.@etl_schema.lk_micro_cross_ref cr
...
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_meas_ab_clean CASCADE;
CREATE TABLE @etl_database.@etl_schema.lk_meas_ab_clean AS
SELECT
    src.subject_id                              AS subject_id,
    src.hadm_id                                 AS hadm_id,
    cr.start_datetime                           AS start_datetime,
...
FROM
    @etl_database.@etl_schema.src_microbiologyevents src
INNER JOIN
    @etl_database.@etl_schema.lk_micro_cross_ref cr
...
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_d_micro_clean CASCADE;
CREATE TABLE @etl_database.@etl_schema.lk_d_micro_clean AS
SELECT
    dm.itemid                                       AS itemid,
    dm.itemid::TEXT                                 AS source_code,
...
FROM
    @etl_database.@etl_schema.src_d_micro dm
UNION ALL
SELECT DISTINCT
...
FROM
    @etl_database.@etl_schema.lk_meas_ab_clean src
WHERE
...
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_d_micro_concept CASCADE;
CREATE TABLE @etl_database.@etl_schema.lk_d_micro_concept AS
SELECT
    dm.itemid                   AS itemid,
...
FROM
    @etl_database.@etl_schema.lk_d_micro_clean dm
LEFT JOIN
...
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_specimen_mapped CASCADE;
CREATE TABLE @etl_database.@etl_schema.lk_specimen_mapped AS
SELECT
    ABS(HASHTEXT(gen_random_uuid()::TEXT))      AS specimen_id,
    src.subject_id                              AS subject_id,
...
FROM
    @etl_database.@etl_schema.lk_specimen_clean src
INNER JOIN
    @etl_database.@etl_schema.lk_d_micro_concept mc
        ON src.spec_itemid = mc.itemid
LEFT JOIN
    @etl_database.@etl_schema.lk_micro_hadm_id hadm
...
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_meas_organism_mapped CASCADE;
CREATE TABLE @etl_database.@etl_schema.lk_meas_organism_mapped AS
SELECT
    ...
    src.trace_id                    AS trace_id
FROM
    @etl_database.@etl_schema.lk_meas_organism_clean src
INNER JOIN
    @etl_database.@etl_schema.lk_d_micro_concept tc
        ON src.test_itemid = tc.itemid
INNER JOIN
    @etl_database.@etl_schema.lk_d_micro_concept sc
        ON src.spec_itemid = sc.itemid
LEFT JOIN
    @etl_database.@etl_schema.lk_d_micro_concept oc
...
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_meas_ab_mapped CASCADE;
CREATE TABLE @etl_database.@etl_schema.lk_meas_ab_mapped AS
SELECT
    ABS(HASHTEXT(gen_random_uuid()::TEXT))      AS measurement_id,
    src.subject_id                              AS subject_id,
    COALESCE(src.hadm_id, hadm.hadm_id)         AS hadm_id,
...
FROM
    @etl_database.@etl_schema.lk_meas_ab_clean src
INNER JOIN
    @etl_database.@etl_schema.lk_d_micro_concept ac
        ON src.ab_itemid = ac.itemid
LEFT JOIN
    @etl_database.@etl_schema.lk_d_micro_concept rc
        ON src.interpretation = rc.source_code
        AND rc.source_vocabulary_id = 'mimiciv_micro_resistance' -- new vocab
LEFT JOIN
    @etl_database.@etl_schema.lk_meas_operator_concept opc -- see lk_meas_labevents.sql
