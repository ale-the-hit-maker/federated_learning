...
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_admissions_clean CASCADE;

CREATE TABLE @etl_database.@etl_schema.lk_admissions_clean AS
SELECT
...
FROM
    @etl_database.@etl_schema.src_admissions src -- adm
;
...
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_transfers_clean CASCADE;

CREATE TABLE @etl_database.@etl_schema.lk_transfers_clean AS
SELECT
    src.subject_id                                  AS subject_id,
...
LEFT JOIN
    @etl_database.@etl_schema.lk_admissions_clean vis -- associate transfers with admissions according to
...
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_services_duplicated CASCADE;

CREATE TABLE @etl_database.@etl_schema.lk_services_duplicated AS
SELECT
    trace_id, COUNT(*) AS row_count
FROM
    @etl_database.@etl_schema.src_services src
GROUP BY
    src.trace_id
HAVING COUNT(*) > 1
;
...
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_services_clean CASCADE;

CREATE TABLE @etl_database.@etl_schema.lk_services_clean AS
SELECT
    src.subject_id                                  AS subject_id,
    src.hadm_id                                     AS hadm_id,
...
FROM
    @etl_database.@etl_schema.src_services src
LEFT JOIN
    @etl_database.@etl_schema.lk_services_duplicated sd
