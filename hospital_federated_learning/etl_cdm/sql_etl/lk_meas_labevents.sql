...
--      see :etl_database.:etl_schema.an_labevents_full
...
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_meas_d_labitems_clean;
CREATE TABLE @etl_database.@etl_schema.lk_meas_d_labitems_clean AS
SELECT
    dlab.itemid                                                 AS itemid, -- for <cdm>.<source_value>
    COALESCE(dlab.loinc_code,
        dlab.itemid::TEXT)                                      AS source_code, -- to join to vocabs
...
FROM
    @etl_database.@etl_schema.src_d_labitems dlab
;
...
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_meas_labevents_clean;
CREATE TABLE @etl_database.@etl_schema.lk_meas_labevents_clean AS
SELECT
    ...
    (regexp_match(src.value, '^(\<=|\>=|\>|\<|=|)'))[1]        AS value_operator,
    (regexp_match(src.value, '[-]?[0-9]+[.]?[0-9]*'))[1]       AS value_number, -- assume "-0.34 etc"
    CASE
...
FROM
    @etl_database.@etl_schema.src_labevents src
INNER JOIN
    @etl_database.@etl_schema.src_d_labitems dlab
...
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_meas_d_labitems_concept;
CREATE TABLE @etl_database.@etl_schema.lk_meas_d_labitems_concept AS
SELECT
...
FROM
    @etl_database.@etl_schema.lk_meas_d_labitems_clean dlab
LEFT JOIN
...
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_meas_labevents_hadm_id;
CREATE TABLE @etl_database.@etl_schema.lk_meas_labevents_hadm_id AS
SELECT
...
FROM
    @etl_database.@etl_schema.lk_meas_labevents_clean src
INNER JOIN
...
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_meas_labevents_mapped;
CREATE TABLE @etl_database.@etl_schema.lk_meas_labevents_mapped AS
SELECT
    ...
    src.itemid                              AS itemid,
    src.itemid::TEXT                        AS source_code, -- change working source code to the rerpresentation
...
FROM
    @etl_database.@etl_schema.lk_meas_labevents_clean src
INNER JOIN
    @etl_database.@etl_schema.lk_meas_d_labitems_concept labc
        ON labc.itemid = src.itemid
LEFT JOIN
    @etl_database.@etl_schema.lk_meas_operator_concept opc
        ON opc.source_code = src.value_operator
LEFT JOIN
    @etl_database.@etl_schema.lk_meas_unit_concept uc
        ON uc.source_code = src.valueuom
LEFT JOIN
    @etl_database.@etl_schema.lk_meas_labevents_hadm_id hadm
