...
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS @etl_database.@etl_schema.tmp_drugIngredientExp;
CREATE TABLE @etl_database.@etl_schema.tmp_drugIngredientExp AS
SELECT
...
FROM @etl_database.@etl_schema.cdm_drug_exposure de
INNER JOIN @etl_database.@etl_schema.voc_drug_strength ds
    ON de.drug_concept_id = ds.drug_concept_id
INNER JOIN @etl_database.@etl_schema.voc_concept_ancestor ca
...
DROP TABLE IF EXISTS @etl_database.@etl_schema.tmp_drugWithDose;
CREATE TABLE @etl_database.@etl_schema.tmp_drugWithDose AS
SELECT
...
FROM @etl_database.@etl_schema.tmp_drugIngredientExp;
...
DROP TABLE IF EXISTS @etl_database.@etl_schema.tmp_cteDoseTarget;
CREATE TABLE @etl_database.@etl_schema.tmp_cteDoseTarget AS
SELECT
    ...
    COALESCE(drug_exposure_end_date,
        ...
        NULLIF(drug_exposure_start_date + INTERVAL '1 day' * (1 * days_supply * (COALESCE(refills, 0) + 1)), drug_exposure_start_date),
...
FROM @etl_database.@etl_schema.tmp_drugWithDose dwd
WHERE
    dose_value <> -1;
...
DROP TABLE IF EXISTS @etl_database.@etl_schema.tmp_cteDoseEndDates_rawdata;
CREATE TABLE @etl_database.@etl_schema.tmp_cteDoseEndDates_rawdata AS
SELECT
    person_id                                       AS person_id,
    ...
    -1                                              AS event_type,
    ROW_NUMBER() OVER (
        PARTITION BY person_id, drug_concept_id, unit_concept_id, CAST(dose_value AS BIGINT)
        ORDER BY drug_exposure_start_date)          AS start_ordinal
FROM @etl_database.@etl_schema.tmp_cteDoseTarget
UNION ALL
...
DROP TABLE IF EXISTS @etl_database.@etl_schema.tmp_cteDoseEndDates_e;
CREATE TABLE @etl_database.@etl_schema.tmp_cteDoseEndDates_e AS
SELECT
    person_id                                                                   AS person_id,
    drug_concept_id                                                             AS drug_concept_id,
    unit_concept_id                                                             AS unit_concept_id,
...
FROM @etl_database.@etl_schema.tmp_cteDoseEndDates_rawdata;
...
DROP TABLE IF EXISTS @etl_database.@etl_schema.tmp_cteDoseEndDates;
CREATE TABLE @etl_database.@etl_schema.tmp_cteDoseEndDates AS
SELECT
    person_id                                       AS person_id,
    drug_concept_id                                 AS drug_concept_id,
    unit_concept_id                                 AS unit_concept_id,
...
FROM @etl_database.@etl_schema.tmp_cteDoseEndDates_e
WHERE
...
DROP TABLE IF EXISTS @etl_database.@etl_schema.tmp_cteDoseFinalEnds;
CREATE TABLE @etl_database.@etl_schema.tmp_cteDoseFinalEnds AS
SELECT
...
FROM @etl_database.@etl_schema.tmp_cteDoseTarget dt
INNER JOIN @etl_database.@etl_schema.tmp_cteDoseEndDates e
...
INSERT INTO @etl_database.@etl_schema.cdm_dose_era
SELECT
    CAST((EXTRACT(EPOCH FROM NOW()) * 1000000 + RANDOM() * 1000000) AS BIGINT) AS dose_era_id,
    person_id                           AS person_id,
    drug_concept_id                     AS drug_concept_id,
    unit_concept_id                     AS unit_concept_id,
    dose_value                          AS dose_value,
    ...
    NULL::TEXT                  AS load_table_id,
    NULL::BIGINT                AS load_row_id
FROM @etl_database.@etl_schema.tmp_cteDoseFinalEnds
GROUP BY
    person_id,
    drug_concept_id,
    unit_concept_id,
