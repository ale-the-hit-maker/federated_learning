...
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_wf_clean CASCADE;

DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_waveform_clean CASCADE;
CREATE TABLE @etl_database.@etl_schema.lk_waveform_clean AS
SELECT
    ...
    CASE
        WHEN EXTRACT(YEAR FROM src.mx_datetime) < pat.anchor_year THEN
            MAKE_TIMESTAMP(
                pat.anchor_year,
                EXTRACT(MONTH FROM src.mx_datetime)::INTEGER,
                EXTRACT(DAY FROM src.mx_datetime)::INTEGER,
                EXTRACT(HOUR FROM src.mx_datetime)::INTEGER,
                EXTRACT(MINUTE FROM src.mx_datetime)::INTEGER,
                EXTRACT(SECOND FROM src.mx_datetime)::INTEGER
            )
        ELSE src.mx_datetime
...
FROM
    @etl_database.@etl_schema.src_waveform_mx src -- wm
INNER JOIN
    @etl_database.@etl_schema.src_waveform_header wh
...
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_wf_hadm_id CASCADE;
CREATE TABLE @etl_database.@etl_schema.lk_wf_hadm_id AS
SELECT
...
FROM
    @etl_database.@etl_schema.lk_waveform_clean src
INNER JOIN
...
DROP TABLE IF EXISTS @etl_database.@etl_schema.lk_meas_waveform_mapped CASCADE;
CREATE TABLE @etl_database.@etl_schema.lk_meas_waveform_mapped AS
SELECT
...
LEFT JOIN
    @etl_database.@etl_schema.voc_concept_relationship vr
...
LEFT JOIN
    @etl_database.@etl_schema.lk_wf_hadm_id hadm
